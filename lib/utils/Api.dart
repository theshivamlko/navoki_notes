...
/// To work on Notes List
String get USER_DATA_API => "https://firestore.googleapis.com/v1/projects/navoki-fc725/databases/(default)/documents/notes/${Utils.userId}/";

/// Register User
String get registerUserApi => "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${AppConstants.API_KEY}";

/// Login Existing User with Email and Password
String get signInApi => "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${AppConstants.API_KEY}";

/// Login Existing User with Token
String get loginWithApiToken => "https://www.googleapis.com/identitytoolkit/v3/relyingparty/getAccountInfo?key=${AppConstants.API_KEY}";

/// Send reset password email to user
String get passwordResetApi => "https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=${AppConstants.API_KEY}";

/// return List of notes of user
Future<List<NoteModel>> getNotes() async {
  try {
    var response = await http.get(Uri.parse(NOTE_API), headers: {"Authorization": "Bearer ${Utils.loginToken}"});
        ...
        if (list != null && !list.isEmpty) {
          for (int i = 0; i < list.length; i++) {
            NoteModel noteModel = NoteModel.store(
                ...
                ((map['documents'][i]['fields'] ?? Map())['title'] ?? Map())['stringValue'],
                ((map['documents'][i]['fields'] ?? Map())['description'] ?? Map())['stringValue'],
                int.parse(((map['documents'][i]['fields'] ?? Map())['colorValue'] ?? Map())['stringValue'] ?? Colors.red.value.toString()));
...
Future<String> signInUser(String email, String password) async {
  try {
    var response = await http.post(Uri.parse(signInApi),
        body: json.encode({AppConstants.EMAIL: email, AppConstants.PASSWORD: password, "returnSecureToken": true}));
    if (response.statusCode == 200) {
      Map map = json.decode(response.body);
...
Future<String> registerUser(String email, String password) async {
  try {
    var response = await http.post(Uri.parse(registerUserApi), body: json.encode({AppConstants.EMAIL: email, AppConstants.PASSWORD: password}));
...
Future<bool> signInWithToken(String token) async {
  try {
    var response = await http.post(Uri.parse(loginWithApiToken), body: json.encode({"idToken": token}));
...
Future<bool> resetPassword(String email) async {
  try {
    var response =
        await http.post(Uri.parse(passwordResetApi), body: json.encode({AppConstants.EMAIL: email, AppConstants.REQUEST_TYPE: "PASSWORD_RESET"}));
    if (response.statusCode == 200) {
      return true;
    }
    return false;
...
Future<bool> sendVerifyEmail(String email) async {
  try {
    var response =
        await http.post(Uri.parse(passwordResetApi), body: json.encode({AppConstants.EMAIL: email, AppConstants.REQUEST_TYPE: "VERIFY_EMAIL"}));
    if (response.statusCode == 200) {
      return true;
    }
    return false;
